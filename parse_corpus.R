...
crawler <- function(n_urls, n_scroll_down, max_sleep_time,
  ...
  cat(sprintf('%s get urls list', Sys.time()))
  i <- 1
  href_list <- 0
  pb <- startpb(0, n_urls)
  ...
  cat(sprintf('%s get content from url of urls list', Sys.time()))
...
main <- function() {
  ...
  option_list <- list(
    make_option('--n_urls', type = 'integer', default = 100), 
    make_option('--n_scroll_down', type = 'integer', default = 5), 
    make_option('--max_sleep_time', type = 'numeric', default = 0.2), 
    make_option('--baike_all_path', type = 'character', default = 'baike_all.rda'), 
    make_option('--baike_para_path', type = 'character', default = 'baike_para.txt'), 
